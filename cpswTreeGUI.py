  ...
  for opt in opts:
    if   opt[0] in ('-a', '--ipAddress') and not useEpics:
      ...
      if not useEpics:
        ...
        print("    --socksProxy <addr>        : connect to any TCP destinations (including connections to an")
        ...
        print("Usage: {} --useEpics|--useEpicsOnly [--recordPrefix=prefix] [--help] yaml_file [root_node]".format(oargs[0]))
        print()
        ...
        print("    --recordPrefix=prefix      : EPICS Record name prefix; must match (non-hashed) prefix set")
        print("                                 on the IOC.")
